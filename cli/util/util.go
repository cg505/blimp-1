...
func GetDockerClient() (*client.Client, error) {
	dockerClient, dcErr := client.NewClientWithOpts(client.FromEnv, client.WithAPIVersionNegotiation())

	// If a custom host is set, don't try any funny business.
	if dcErr != nil || dockerClient.DaemonHost() != client.DefaultDockerHost {
		return dockerClient, dcErr
	}
	...
	// It doesn't exist. If we're in WSL, see if we should use the TCP socket instead
